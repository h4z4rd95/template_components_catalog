...
npm run build        # sync manifest → static-export the framework apps → splice into docs/
                       # …then rebuild the downloadable source archive
...
brand + download + menu on a phone, with the rest of the controls moved into the drawer). It offers
the full project as one ZIP and the GitHub repository as the always-works alternative.

`docs/download/catalog-source.zip` is a **build artifact**, not a committed binary: `npm run build`
ends by generating it from `git ls-files`, so the archive is exactly the tracked source — the
catalogue, both framework apps, the vanilla track, the shared package, the scripts and the docs —
at the revision being deployed. `docs/vision/**` (the generated motion reel, ~26 MB),
`docs/framework/**` (regenerated by the build) and nested archives are excluded; that leaves
**99 files ≈ 813 KB zipped**. The button states those real numbers, read from
`docs/data/download.json`, and updates them in either language. Run `npm run bundle` to rebuild the
archive on its own.
